...
pub struct CfbEncryption<T: BlockCipherEncryption<BLOCKSIZE>, const BLOCKSIZE: usize> {
    primitive: T,
    iv: Array<u8, BLOCKSIZE>,
    pos: usize,
}

impl<T: BlockCipherEncryption<B>, const B: usize> CfbEncryption<T, B> {
...
impl<T: BlockCipherEncryption<B>, const B: usize> StreamEncryption for CfbEncryption<T, B> {
    fn encrypt(&mut self, data: &mut [u8]) {
        ...
        while encrypted < data.len() {
            ...
            let xored = xor_buffers(&mut data[encrypted..], op_slice);
...
pub struct CfbDecryption<T: BlockCipherEncryption<BLOCKSIZE>, const BLOCKSIZE: usize> {
    primitive: T,
    iv: Array<u8, BLOCKSIZE>,
    pos: usize,
}

impl<T: BlockCipherEncryption<B>, const B: usize> CfbDecryption<T, B> {
    pub(super) fn new(primitive: T, iv: [u8; B]) -> Self {
        Self {
            primitive,
            iv: Array::from(iv),
...
impl<T: BlockCipherEncryption<B>, const B: usize> StreamDecryption for CfbDecryption<T, B> {
    fn decrypt(&mut self, data: &mut [u8]) {
        ...
        while decrypted < data.len() {
            ...
            xor_buffers(dec_slice, op_slice);
