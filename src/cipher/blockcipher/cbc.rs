...
impl<T: PrimitiveEncryption<B>, const B: usize> BlockCipherEncryption for CbcEncryption<T, B> {
...
impl<T: PrimitiveEncryption<B>, const B: usize> ioWrite for CbcEncryption<T, B> {
...
impl<T: PrimitiveDecryption<B>, const B: usize> CbcDecryption<T, B> {
    ...
    fn process_buffer(&mut self) {
        ...
        let new_iv = {
            let mut iv = FixedBuffer::new();
            iv.push_slice(self.buffer.as_ref());
            iv
        };
...
impl<T: PrimitiveDecryption<B>, const B: usize> BlockCipherDecryption for CbcDecryption<T, B> {
    fn finalize(&mut self) -> BlockCipherResult {
...
impl<T: PrimitiveDecryption<B>, const B: usize> ioWrite for CbcDecryption<T, B> {
