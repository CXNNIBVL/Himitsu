...
{
    fn setup(arc: &Arc<ShardedLock<T>>, threads: usize) -> (Sender<InData<B>>, Receiver<OutData<B>>) {
        ...
        for _ in 0..threads {
            ...
            thread::spawn(move || {
                ...
                while let Ok(InData { mut mut_block, xor_pre, xor_post, id }) = rx.recv() {
                    let cipher = arc.as_ref().read().unwrap();
                    cipher.encrypt(&mut mut_block, xor_pre.as_ref(), xor_post.as_ref());
                    tx.send(OutData { block: mut_block, id }).unwrap();
                }
            });
        }
    ...
    pub fn new(primitive: T, threads: usize) -> Self {
        ...
        Self { tx, rx, block_count: 0, ph: std::marker::PhantomData{} }
    }

    pub fn put(&mut self, mut_block: [u8;B], xor_pre: Option<[u8;B]>, xor_post: Option<[u8;B]>) {
        let in_data = InData { mut_block, xor_pre, xor_post, id: self.block_count };
